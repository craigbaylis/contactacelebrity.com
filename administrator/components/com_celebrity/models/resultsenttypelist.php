 ...
 */
class CelebrityModelResultsenttypelist extends JModel{
	/**
	 * Resultsenttypelist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__celebrity_result_sent_type` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__celebrity_result_sent_type` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__celebrity_result_sent_type` ORDER BY `$fieldName`');
		$options = array();
