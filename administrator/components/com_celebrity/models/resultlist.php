 ...
 */
class CelebrityModelResultlist extends JModel{
	/**
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__celebrity_result` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,date_sent,received_type_id,date_received,comments,address_id,quality_id,received_image_id,created_by_id,published,date_created'); // array('id', 'ordering', 'published'); 

		// retrive ordering info
		$filter_order = $app->getUserStateFromRequest('com_celebrityfilter_order', 'filter_order', $default_order_field);
		$filter_order_Dir = strtoupper($app->getUserStateFromRequest('com_celebrityfilter_order_Dir', 'filter_order_Dir', 'ASC'));
	...
	private function _buildQueryWhere() {
		...
		$search = $app->getUserStateFromRequest('com_celebritysearch', 'search', '');
		
	    if (!$search) return '';
		
		$allowedSearch = explode(',', 'comments'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__celebrity_result` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__celebrity_result` ORDER BY `$fieldName`');
		$options = array();
