 ...
 */
class CelebrityModelResultreceivedimagelist extends JModel{
	/**
	 * Resultreceivedimagelist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__celebrity_result_received_image` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,title,caption,file_name,published,date_created'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'title,caption'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__celebrity_result_received_image` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__celebrity_result_received_image` ORDER BY `$fieldName`');
		$options = array();
