...
class CelebrityModelResult extends JModel{
	...
	public function &getData(){
		// Load the data
		if (empty( $this->_data )) {
			$query = 'SELECT * FROM `#__celebrity_result` WHERE `id` = '.$this->_id;
...
// mcm code 
		...
		$data['received_type_id'] = JRequest::getVar('received_type_id', '', 'post', 'int');
		$data['address_id'] = JRequest::getVar('address_id', '', 'post', 'int');
		$data['quality_id'] = JRequest::getVar('quality_id', '', 'post', 'int');
		$data['created_by_id'] = JRequest::getVar('created_by_id', '', 'post', 'int');
		$data['published'] = JRequest::getVar('published', '', 'post', 'int');
        $date = new JDate();
        $data['date_created'] = $date->toMySQL();
// mcm code 
