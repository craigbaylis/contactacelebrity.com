 ...
 */
class CelebrityModelResultsentlist extends JModel{
	/**
	 * Resultsentlist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__celebrity_result_sent` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,result_id,sent_type_id,date_created'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', ''); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__celebrity_result_sent` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__celebrity_result_sent` ORDER BY `$fieldName`');
		$options = array();
