 ...
 */
class CelebrityControllerResultsentlist extends CelebrityController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'resultsent' );
		JRequest::setVar( 'layout', 'default'  );
		JRequest::setVar( 'hidemainmenu', 1 );
		
		$view =& $this->getView('resultsent', 'html');
	...
	public function save(){
		$model = $this->getModel('resultsentlist'); 
		...
		$link = 'index.php?option=com_celebrity&controller=resultsentlist';
	...
	public function remove(){
		$model = $this->getModel('resultsent');
		if(!$model->delete()) {
		...
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultsentlist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_sent SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultsentlist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_sent SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'resultsent' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_celebrity&view=resultsentlist' );
	}
	*/
