 ...
 */
class CelebrityControllerResultreceivedimagelist extends CelebrityController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'resultreceivedimage' );
		JRequest::setVar( 'layout', 'default'  );
		JRequest::setVar( 'hidemainmenu', 1 );
		
		$view =& $this->getView('resultreceivedimage', 'html');
	...
	public function save(){
		$model = $this->getModel('resultreceivedimagelist'); 
		...
		$link = 'index.php?option=com_celebrity&controller=resultreceivedimagelist';
	...
	public function remove(){
		$model = $this->getModel('resultreceivedimage');
		if(!$model->delete()) {
		...
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultreceivedimagelist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_received_image SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultreceivedimagelist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_received_image SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'resultreceivedimage' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_celebrity&view=resultreceivedimagelist' );
	}
	*/
