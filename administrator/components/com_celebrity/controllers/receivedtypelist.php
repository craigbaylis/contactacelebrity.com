 ...
 */
class CelebrityControllerReceivedtypelist extends CelebrityController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'resultreceivedtype' );
		JRequest::setVar( 'layout', 'default'  );
		JRequest::setVar( 'hidemainmenu', 1 );
		
		$view =& $this->getView('resultreceivedtype', 'html');
	...
	public function save(){
		$model = $this->getModel('receivedtypelist'); 
		...
		$link = 'index.php?option=com_celebrity&controller=receivedtypelist';
	...
	public function remove(){
		$model = $this->getModel('resultreceivedtype');
		if(!$model->delete()) {
		...
		$this->setRedirect( 'index.php?option=com_celebrity&controller=receivedtypelist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_received_type SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_celebrity&controller=receivedtypelist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_received_type SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'resultreceivedtype' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_celebrity&view=receivedtypelist' );
	}
	*/
