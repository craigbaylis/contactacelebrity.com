 ...
 */
class CelebrityControllerResultsenttypelist extends CelebrityController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'resultsenttype' );
		JRequest::setVar( 'layout', 'default'  );
		JRequest::setVar( 'hidemainmenu', 1 );
		
		$view =& $this->getView('resultsenttype', 'html');
	...
	public function save(){
		$model = $this->getModel('resultsenttypelist'); 
		...
		$link = 'index.php?option=com_celebrity&controller=resultsenttypelist';
	...
	public function remove(){
		$model = $this->getModel('resultsenttype');
		if(!$model->delete()) {
		...
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultsenttypelist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_sent_type SET published = 1 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_celebrity&controller=resultsenttypelist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__celebrity_result_sent_type SET published = 0 WHERE `id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'resultsenttype' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_celebrity&view=resultsenttypelist' );
	}
	*/
